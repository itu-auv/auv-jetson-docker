...
if [ $IMAGE == "noetic-base" ]; then
  ...
  sudo docker build -t $IMAGE_TAG -f $DOCKERFILE --platform linux/arm64 --build-arg ROS_PKG=ros_base --build-arg BASE_IMAGE=$BASE_IMAGE .
  ...
  sudo docker build -t $IMAGE_TAG -f $DOCKERFILE --platform linux/arm64 --build-arg ROS_PKG=desktop_full --build-arg BASE_IMAGE=$BASE_IMAGE .
